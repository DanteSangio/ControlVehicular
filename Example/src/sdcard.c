...
unsigned char SD_writeSingleBlock(char *inputbuffer,uint32_t startBlock)
{
    ...
    Chip_SSP_WriteFrames_Blocking(0xfe);     //Send start block token 0xfe (0x11111110)

    for(i=0; i<512; i++)    //send 512 bytes data
        Chip_SSP_WriteFrames_Blocking(inputbuffer[i]);
...
uint8_t init_SdCard(uint8_t *cardType)
{
    ...
    for(i=0;i<10;i++)
    	Chip_SSP_WriteFrames_Blocking(0xff);   //80 clock pulses spent before sending the first command
